...
# SO100 Real Robot with Dual Camera Support
import time
...
class SO100DualCamRobot:
    def __init__(self, calibrate=False, enable_camera=False, camera_right_index=9, camera_top_index=3):
    ...
    def connect(self):
        ...
        print("================> SO100 Robot with dual cameras is fully connected =================")
...
class Gr00tDualCamInferenceClient:
    def __init__(
        self,
        host="localhost",
        port=5555,
    ...
    def get_action(self, images, state):
        ...
        Args:
            images: Dictionary with 'right' and 'top' camera images
            state: Robot state
        """
        obs_dict = {
...
if __name__ == "__main__":
    ...
    if USE_POLICY:
        client = Gr00tDualCamInferenceClient(
            host=args.host,
            port=args.port,
            language_instruction="Pick up the items from the white plate and place them in the black recepient.",
        )

        robot = SO100DualCamRobot(
            calibrate=False, 
            enable_camera=True, 
        ...
        print("Playback from dataset not supported in dual camera mode. Please use --use_policy") 
